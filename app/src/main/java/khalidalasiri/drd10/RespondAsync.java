...
class RespondAsync extends AsyncTask<String, Void, String[]> {
    ...
    public RespondAsync(List<HttpCookie> token, Context context, String tableURL, String tableName) {
        this.token = token;
    ...
    protected String[] doInBackground(String... strings) {
        ...
        try {
            ...
            String include= "&include=Respond";
            builder = connection(key,columnName,primaryKey,tableURL,include);

            JSONObject root = new JSONObject(builder.toString());
            JSONObject jsonObject = root.getJSONObject(tableName);
            ...
            if (records.isNull(0)) {
                ...
                result[0] = "OK";
                result[1] = records.getJSONArray(0).get(1).toString() ; // The ReportDate
                result[2] = records.getJSONArray(0).get(2).toString() ; // The BloodPressure
                result[3] = records.getJSONArray(0).get(3).toString() ; // The BloodGlucoseAnalysis
                result[4] = records.getJSONArray(0).get(4).toString() ; // The HeartRate

                JSONObject jsonObject1 = root.getJSONObject("Respond");
                JSONArray records1 = jsonObject1.getJSONArray("records");

                if (!records1.isNull(0)) {
                    result[5] = records1.getJSONArray(0).get(1).toString(); // The RespondDate
                    result[6] = records1.getJSONArray(0).get(2).toString(); // The RespondContent
                }
